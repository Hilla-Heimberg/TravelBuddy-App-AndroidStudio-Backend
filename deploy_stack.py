...
def deploy_code_package(now_ms: int) -> None:
    bucket_name = "travel-buddy-code-bucket"
    key = f"travel-buddy-{now_ms}-code-package.zip"
...
def get_lambdas_urls(stack_name_to_check: str) -> Tuple[Optional[str], Optional[str]]:
    cloudformation = boto3.resource("cloudformation", region_name=AWS_REGION)
    stack = cloudformation.Stack(stack_name_to_check)
    section_uploader_url = None
    section_getter_url = None
    for output in stack.outputs:
        if output["OutputKey"] == "SectionContentGetterLambdaURL":
            section_getter_url = output["OutputValue"]
        if output["OutputKey"] == "SectionContentUploaderLambdaURL":
            section_uploader_url = output["OutputValue"]

    return section_uploader_url, section_getter_url
...
def main() -> None:
    ...
    try:
        print("start to deploy code bucket")
        deploy_cloudformation_template(
            now_ms=now_ms,
            stack_name="travel-buddy-buckets",
            template_file_path="./cloudformation/code_bucket.yaml",
        )

        deploy_code_package(now_ms=now_ms)

        print("start to deploy travel buddy")
        deploy_cloudformation_template(
            now_ms=now_ms,
            stack_name="travel-buddy-resources",
            template_file_path="./cloudformation/travel_buddy.yaml",
        )

        section_content_uploader_url, section_content_getter_url = get_lambdas_urls("travel-buddy-resources")
        print(f"Section content uploader lambda URL: {section_content_uploader_url}")
        print(f"Section content getter lambda URL: {section_content_getter_url}")
